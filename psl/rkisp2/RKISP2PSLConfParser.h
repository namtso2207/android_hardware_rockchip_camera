...
class RKISP2PSLConfParser : public IPSLConfParser {
public:
...
private:
    RKISP2PSLConfParser(RKISP2PSLConfParser const&);
    RKISP2PSLConfParser& operator=(RKISP2PSLConfParser const&);

private:
    static IPSLConfParser *sInstance; // the singleton instance

    RKISP2PSLConfParser(std::string& xmlName, const std::vector<SensorDriverDescriptor>& sensorNames);
    virtual ~RKISP2PSLConfParser();
    ...
    void handleRKISP2SensorInfo(const char *name, const char **atts);
