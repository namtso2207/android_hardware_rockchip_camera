 ...
 */
 ...
 #define LOG_TAG "RKISP2CameraCapInfo"
...
RKISP2CameraCapInfo::RKISP2CameraCapInfo(SensorType type):
    mSensorType(type),
...
RKISP2CameraCapInfo::~RKISP2CameraCapInfo()
{
    CLEAR(mFov);
    delete mGCMNodes;
}
...
const RKISP2CameraCapInfo *getRKISP2CameraCapInfo(int cameraId)
{
    ...
    return (RKISP2CameraCapInfo *)(PlatformData::getCameraCapInfo(cameraId));
}

const string RKISP2CameraCapInfo::getMediaCtlEntityName(string type) const
{
    LOGI("@%s", __FUNCTION__);
...
const std::vector<string> RKISP2CameraCapInfo::getMediaCtlEntityNames(string type) const
{
    LOGI("@%s", __FUNCTION__);
...
const std::string RKISP2CameraCapInfo::getMediaCtlEntityType(std::string name) const
{
    LOGI("@%s", __FUNCTION__);
